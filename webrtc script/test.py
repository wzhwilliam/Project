...
def launch_meet(args):
	...
	res = Popen('/usr/bin/google-chrome', shell=True)
	time.sleep(2)
	...
	if guibot.exists('allow_mic.png', timeout=1):
		guibot.click('allow_mic.png')
		
	if guibot.exists('allow_mic.png', timeout=1):
		guibot.click('allow_mic.png')
	
	if guibot.exists('maximize.png', timeout=1):
		guibot.click('maximize.png')
	
	#Added by Floris begin
	if guibot.exists('meet_name.png', timeout=5):
		guibot.click('meet_name.png')

	pyautogui.write('bot')
	pyautogui.hotkey('enter')

	if guibot.exists('meet_ask_to_join.png', timeout=5):
		guibot.click('meet_ask_to_join.png')
	#Added by Flores end

	if guibot.exists('meet_join_now.png', timeout=5):
